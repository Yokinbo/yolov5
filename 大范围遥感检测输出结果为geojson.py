...
# ============================================================
# YOLOv5 大幅面 GeoTIFF 推理：滑窗切片 + 重叠 +（跨切片）全局NMS + 输出 GeoJSON
# 已加入 tqdm 进度条
# ============================================================
...
DEFAULT_SOURCE_TIF = "/mnt/f/金三角原始下载影像/简单测试可删/矩形区域/Level16/矩形区域.tif"
DEFAULT_OUTDIR     = "/mnt/f/金三角原始下载影像/简单测试可删/矩形区域/Level16/推理结果"
...
def parse_args():
    p = argparse.ArgumentParser("YOLOv5 Large GeoTIFF Inference (tiling + global NMS + GeoJSON)")
    ...
    # 注意：store_true 的 default 建议保持 False；如想默认 True，可在 DEFAULT_AGNOSTIC 设 True
...
def load_model(weights, device="", half=False):
    # 注意：这些 import 依赖你已经把 yolov5_dir 加入 sys.path（见 main 里 add_yolov5_to_path）
...
def main():
    ...
    with rasterio.open(args.source) as src:
        ...
        pbar = tqdm(total=total_tiles, desc="YOLOv5 tiled inference", ncols=100)
