# yolov5_big_tif_infer_fast_uint8.py
# ============================================================
# YOLOv5 大幅面 GeoTIFF 推理：滑窗切片 + 重叠 +（跨切片）全局NMS
# 输出：GeoJSON + Shapefile（同时输出） + 像素坐标JSON
# ✅ 优化：uint8 快路径（不做 percentile 拉伸） + cudnn benchmark + tqdm
# ============================================================
...
DEFAULT_OUTDIR     = "/mnt/f/金三角原始下载影像/简单测试可删/矩形区域/Level17/推理结果测试可删"
...
DEFAULT_HALF       = False     # ✅ 想提速可改 True（FP16）
...
def parse_args():
    ...
    p.add_argument("--yolov5-dir", type=str, default=DEFAULT_YOLOV5_DIR, help="Path to local yolov5 repo root")
    p.add_argument("--weights", type=str, default=DEFAULT_WEIGHTS, help="Path to trained .pt weights")
    p.add_argument("--source", type=str, default=DEFAULT_SOURCE_TIF, help="Path to input GeoTIFF (.tif)")
    p.add_argument("--outdir", type=str, default=DEFAULT_OUTDIR, help="Output directory")

    p.add_argument("--imgsz", type=int, default=DEFAULT_IMGSZ, help="Inference size for each tile (square).")
    p.add_argument("--tile", type=int, default=DEFAULT_TILE, help="Sliding window tile size on original image (pixels).")
    p.add_argument("--overlap", type=float, default=DEFAULT_OVERLAP, help="Tile overlap ratio, e.g. 0.25 means 25% overlap.")
    p.add_argument("--conf-thres", type=float, default=DEFAULT_CONF, help="Confidence threshold")
    p.add_argument("--iou-thres", type=float, default=DEFAULT_IOU, help="IOU threshold for NMS")
    p.add_argument("--device", type=str, default=DEFAULT_DEVICE, help="cuda device, i.e. 0 or 0,1,2,3 or cpu")
    p.add_argument("--classes", type=str, default=DEFAULT_CLASSES, help="Filter by class: e.g. '0,2,3' (optional)")
    p.add_argument("--agnostic-nms", action="store_true", default=DEFAULT_AGNOSTIC, help="class-agnostic NMS (if set)")
    p.add_argument("--half", action="store_true", default=DEFAULT_HALF, help="use FP16 if supported (if set)")
    p.add_argument("--max-det", type=int, default=DEFAULT_MAX_DET, help="max detections per tile before merge")

    p.add_argument("--band-index", type=str, default=DEFAULT_BANDS, help="Bands to use for RGB, 1-based index")
...
def read_tile_as_rgb(src, window: Window, band_ids):
    """
    ✅ 优化：如果影像本来就是 uint8（你截图就是 8-bit unsigned），直接返回，不做 percentile
    """
    ...
    if arr.dtype == np.uint8:
        return np.transpose(arr, (1, 2, 0))  # HWC uint8

    # 慢路径：uint16/float 才拉伸
    rgb = []
...
def main():
    ...
    if len(band_ids) != 3:
        raise ValueError("--band-index 必须是 3 个波段，例如 '1,2,3'")

    model, device, stride, names = load_model(args.weights, device=args.device, half=args.half)
